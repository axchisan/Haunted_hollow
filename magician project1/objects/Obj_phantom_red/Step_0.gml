...
if (abs(player_x - x) > abs(player_y - y)) {
    if (player_x > x) {
        sprite_index = Spr_phantom_red_right;
    } else {
        sprite_index = Spr_phantom_red_left;
    }
} else {
    if (player_y > y) {
        sprite_index = Spr_phantom_red_down;
    } else {
        sprite_index = Spr_phantom_red_up;
    }
}
...
if (cooldown_timer > 0) {
    ...
    var bullet = instance_create_layer(x, y, "Bullets", Obj_bullet_phantom_red);
